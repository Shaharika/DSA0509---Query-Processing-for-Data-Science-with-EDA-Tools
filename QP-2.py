...
# Group by EMPLOYEE_ID and count the number of unique JOB_IDs for each employee
job_counts = df.groupby('EMPLOYEE_ID')['JOB_ID'].nunique()
# Filter employees with two or more jobs
multiple_jobs_employees = job_counts[job_counts >= 2].index
# Display the result
result = df[df['EMPLOYEE_ID'].isin(multiple_jobs_employees)][['EMPLOYEE_ID']].drop_duplicates()
print(result)
